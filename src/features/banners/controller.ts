...
import { bannerCreateSchema, bannerUpdateSchema } from "./schema";
...
export class BannerController {
  ...
  async create(_req: Request): Promise<Response> {
    try {
      ...
      const validation = validateBody(input, bannerCreateSchema);
  ...
  async update(_req: Request, params: Record<string, string>): Promise<Response> {
    try {
      ...
      const validationBody = validateBody(inputBody, bannerUpdateSchema);
